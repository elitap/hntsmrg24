...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    if np.any(shape != new_shape):
        # eli was here
        print('eli was here for the miccai dataset we are not skipping seperate z and casting to float')
...
def resample_data_or_seg_dilated_premask_to_shape(data: Union[torch.Tensor, np.ndarray],
                                  new_shape: Union[Tuple[int, ...], List[int], np.ndarray],
                                  current_spacing: Union[Tuple[float, ...], List[float], np.ndarray],
                                  new_spacing: Union[Tuple[float, ...], List[float], np.ndarray],
                                  is_seg: bool = False,
                                  order: int = 3, order_z: int = 0,
                                  force_separate_z: Union[bool, None] = False,
                                  seg_channel: Union[int, None] = None,
                                  separate_z_anisotropy_threshold: float = ANISO_THRESHOLD):
    """
    needed for segmentation export. Stupid, I know
    """
    ...
    data_reshaped = resample_data_or_seg_dilated_premask(data, new_shape, is_seg, axis, order, do_separate_z, order_z=order_z, seg_channel=seg_channel)
    return data_reshaped


def resample_data_or_seg_dilated_premask(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
                         is_seg: bool = False, axis: Union[None, int] = None, order: int = 3,
                         do_separate_z: bool = False, order_z: int = 0, dtype_out = None, seg_channel: Union[int, None] = None):
    """
    separate_z=True will resample with order 0 along z
    :param data:
    :param new_shape:
    ...
    if np.any(shape != new_shape):
        ...
        print(f"{np.unique(reshaped_final[seg_channel], return_counts=True)=}")
        return reshaped_final
    else:
        # print("no resampling necessary")
        return data
