...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        if configuration_manager.network is not None:
            if configuration_manager.network == "TransUNet":
                import sys
                sys.path.append(configuration_manager.module_path)  # Replace '/path/to/TransUNet' with the actual path to the TransUNet module

                from TransUNet.networks.vit_seg_modeling import VisionTransformer as ViT_seg
                from TransUNet.networks.vit_seg_modeling import CONFIGS as CONFIGS_ViT_seg

                # Rest of the code remains unchanged
                config_vit = CONFIGS_ViT_seg[configuration_manager.vit_name]
                config_vit.pretrained_path = os.path.join(configuration_manager.module_path, 
                                                            configuration_manager.pretrained_weights)
                ...
                if configuration_manager.vit_name.find('R50') != -1:
                    config_vit.patches.grid = (int(img_size / vit_patches_size), int(img_size / vit_patches_size))
                net = ViT_seg(config_vit, img_size=img_size, num_classes=config_vit.n_classes).to(self.device)
                # net.load_from(weights=np.load(config_vit.pretrained_path))
                network = net
                print("created transunet....!!")

            elif configuration_manager.network == "unetr":
                network = UNETR(
                        in_channels=num_input_channels, #1,
                        out_channels=plans_manager.get_label_manager(dataset_json).num_segmentation_heads, #8
                        img_size=configuration_manager.patch_size, #(96, 96, 96),
                        feature_size=16,
                        hidden_size=768,
                        mlp_dim=3072,
                        num_heads=12,
            ...
            elif configuration_manager.network.startswith("swinunetr"):
                network = SwinUNETR(
                    img_size=configuration_manager.patch_size, #(96,96,96),
                    in_channels=num_input_channels, #1,
                    out_channels=plans_manager.get_label_manager(dataset_json).num_segmentation_heads, #8,
                    feature_size=48,
                    use_checkpoint=True,
                    use_v2=configuration_manager.network.endswith("v2"),
                    ).to(self.device)
            elif configuration_manager.network == "swinsmt":
                import sys
                sys.path.append(configuration_manager.module_path)  # Replace '/path/to/TransUNet' with the actual path to the TransUNet module
            ...
            elif configuration_manager.network == "uxnet":
                import sys
                sys.path.append(configuration_manager.module_path)

                from uxNet.networks.UXNet_3D.network_backbone import UXNET
                network = UXNET(
                    in_chans=num_input_channels,
                    out_chans=plans_manager.get_label_manager(dataset_json).num_segmentation_heads,
                    depths=[2, 2, 2, 2],
            ...
            elif configuration_manager.network == "nnformer":
                import sys
                sys.path.append(configuration_manager.module_path)

                from uxNet.networks.nnFormer.nnFormer_seg import nnFormer

                network = nnFormer(input_channels=num_input_channels, num_classes=plans_manager.get_label_manager(dataset_json).num_segmentation_heads).to(self.device)
    ...
    # added for swinuntr we need to be in inference mode before param loading!
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                # print("workon shape:", workon.shape)

                # print(f"{self.configuration_manager.channel_to_dilate=}")
                if self.configuration_manager.patch_skipping:
                    # !!!!! this is already done by the preprocessor in this case!!!!!
                    # due to the [None] above we have to access the first dimension again
                    # fg_mask = workon[0, self.configuration_manager.channel_to_dilate].cpu().numpy().astype(np.uint8)
                    # print("unique elements with count in fg_mask:", np.unique(fg_mask, return_counts=True))
                    # print("fg_mask shape:", fg_mask.shape)
                    # foreground_labels = self.label_manager.foreground_labels
                    # fg_mask = DilatedPremaskAsFgPreprocessor.dilate_mask(fg_mask, len(self.configuration_manager.spacing), foreground_labels, self.configuration_manager.dilationradius)
                    # print("unique elements with count in fg_mask after dilation:", np.unique(fg_mask, return_counts=True))
                    # sum_fg_mask = workon[0, self.configuration_manager.channel_to_dilate].sum()

                    #print(f"{sum_fg_mask=}, {self.configuration_manager.dilationradius=}")
                    if workon[0, self.configuration_manager.channel_to_dilate].sum() == 0:
                        print("No foreground mask found. Skipping this patch.")
                        # set everything to background
                        prediction = torch.nn.functional.one_hot(torch.zeros_like(workon[0, 0], dtype=torch.int64), num_classes=self.label_manager.num_segmentation_heads).type(torch.half)
                        # print("prediction shape after oh to create background res:", prediction.shape)
                        # correct shape to channel batch channel ... should work for 2d and 3d
                        prediction = rearrange(prediction, '... c -> c ...').to(results_device)
                    else:
                        # remove we dont want to train with it!!!
                        # workon = np.delete(workon, self.configuration_manager.channel_to_dilate, axis=1)
                        # Split the tensor into two parts: before and after the channel to remove
                        if self.configuration_manager.rem_dilchannel:
                            left = workon[:, :self.configuration_manager.channel_to_dilate]
                            right = workon[:, self.configuration_manager.channel_to_dilate+1:]

                            # Concatenate the two parts along the channel dimension
                            workon = torch.cat((left, right), dim=1)
