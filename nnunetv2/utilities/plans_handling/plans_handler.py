...
class ConfigurationManager(object):
    ...
    # eli was here
    @property
    def dilationradius(self) -> int:
        if "dilationradius" not in self.configuration:
            return 0
        return self.configuration['dilationradius']
    
    @property
    def channel_to_dilate(self) -> Union[int, None]:
        if "channel_to_dilate" not in self.configuration:
            return None
        return self.configuration['channel_to_dilate']
    
    @property
    def rem_dilchannel(self) -> Union[bool, None]:
        if "rem_dilchannel" not in self.configuration:
            return False
        return self.configuration['rem_dilchannel']
    
    @property
    def infer(self) -> Union[bool, None]:
        if "infer" not in self.configuration:
            return False
        return self.configuration['infer']
    
    @property
    def patch_skipping(self) -> Union[bool, None]:
        if "patch_skipping" not in self.configuration:
            return False
        return self.configuration['patch_skipping']
    
    @property
    def fg_oversampling(self) -> Union[float, None]:
        if "fg_oversampling" not in self.configuration:
            return None
        return self.configuration['fg_oversampling']
    ...
    def module_path(self) -> str | None:
        if 'module_path' in self.configuration:
            return self.configuration['module_path']
        return None
       
    @property
    def vit_name(self) -> str | None:
        if 'vit_name' in self.configuration:
            return self.configuration['vit_name']
        return None
    
    @property
    def pretrained_weights(self) -> str | None:
        if 'pretrained_weights' in self.configuration:
            return self.configuration['pretrained_weights']
        return None
