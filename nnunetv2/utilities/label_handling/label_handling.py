...
def determine_num_input_channels(plans_manager: PlansManager,
    ...
    if config_manager.channel_to_dilate is not None and config_manager.rem_dilchannel is True:
        print(f'Using channel {config_manager.channel_to_dilate} only for foreground sampling so we dont need to add it to the input channels')
        num_modalities -= 1
