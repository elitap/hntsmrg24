...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if self.configuration_manager.fg_oversampling is not None:
            print(f"WARNING: fg_oversampling is set in the configuration file. This will overwrite the default value to {self.configuration_manager.fg_oversampling}")
            self.oversample_foreground_percent = self.configuration_manager.fg_oversampling
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50
        self.num_epochs = 1000
        self.current_epoch = 0
        self.enable_deep_supervision = True
        # to be sure in case this is checked somewhere
        if self.configuration_manager.network in ["TransUNet", "swinunetr", "swinunetrv2", "unetr", "swinsmt", "uxnet", "nnformer"]:
    ...
    def initialize(self):
        if not self.was_initialized:
            self.num_input_channels = determine_num_input_channels(self.plans_manager, self.configuration_manager,
                                                                   self.dataset_json)
            if self.configuration_manager.network is not None:
                if self.configuration_manager.network == "TransUNet":
                    import sys
                    sys.path.append(self.configuration_manager.module_path)  # Replace '/path/to/TransUNet' with the actual path to the TransUNet module

                    from TransUNet.networks.vit_seg_modeling import VisionTransformer as ViT_seg
                    from TransUNet.networks.vit_seg_modeling import CONFIGS as CONFIGS_ViT_seg

                    # Rest of the code remains unchanged
                    config_vit = CONFIGS_ViT_seg[self.configuration_manager.vit_name]
                    config_vit.pretrained_path = os.path.join(self.configuration_manager.module_path, 
                                                            self.configuration_manager.pretrained_weights)
                    img_size = self.configuration_manager.patch_size
                    # assert if image size is not quadratic
                    assert img_size[0] == img_size[1], 'Image size should be quadratic'
                    img_size = img_size[0]

                    config_vit.n_classes = 8
                    config_vit.n_skip = 3 # set to 0 if training non res variant
                    vit_patches_size = 16

                    if self.configuration_manager.vit_name.find('R50') != -1:
                        config_vit.patches.grid = (int(img_size / vit_patches_size), int(img_size / vit_patches_size))
                    net = ViT_seg(config_vit, img_size=img_size, num_classes=config_vit.n_classes).to(self.device)
                    net.load_from(weights=np.load(config_vit.pretrained_path))
                    self.network = net

                elif self.configuration_manager.network == "unetr":
                    net = UNETR(
                            in_channels=self.num_input_channels, #1,
                            out_channels=self.label_manager.num_segmentation_heads, #8
                            img_size=self.configuration_manager.patch_size, #(96, 96, 96),
                            feature_size=16,
                            hidden_size=768,
                            mlp_dim=3072,
                            num_heads=12,
                    ...
                    # info: https://github.com/Project-MONAI/research-contributions/tree/main/UNETR/BTCV
                    # pretrained weights are from: https://drive.google.com/file/d/1kR5QuRAuooYcTNLMnMj80Z9IgSs8jtLO/view?usp=sharing (Hatamizadeh et al.)
                    if self.configuration_manager.pretrained_weights is not None:
                        #TODO fix this
                        model_dict = torch.load(self.configuration_manager.pretrained_weights)
                        del model_dict['out.conv.conv.weight']
                        del model_dict['out.conv.conv.bias']
                        
                        # ok lets not be to strict:
                        ...
                        print(f"Using pretrained UNETR backbone weights! {self.configuration_manager.pretrained_weights}")

                    self.network = net.to(self.device)

                elif self.configuration_manager.network.startswith("swinunetr"):
                    net = SwinUNETR(
                    img_size=self.configuration_manager.patch_size, #(96,96,96),
                    in_channels=self.num_input_channels, #1,
                    out_channels=self.label_manager.num_segmentation_heads, #8,
                    feature_size=48,
                    use_checkpoint=True,
                    use_v2=self.configuration_manager.network.endswith("v2"),
                    ).to(self.device)
                    
                    # info: https://github.com/Project-MONAI/research-contributions/tree/main/SwinUNETR/BTCV
                    # pretrained weights are from: https://github.com/Project-MONAI/MONAI-extra-test-data/releases/download/0.8.1/model_swinvit.pt (Tang et al.)
                    try:
                        model_dict = torch.load(self.configuration_manager.pretrained_weights)
                        ...
                        raise ValueError("Self-supervised pre-trained weights not available for" + str(self.configuration_manager.pretrained_weights))
                    
                    self.network = net.to(self.device)
                elif self.configuration_manager.network == "swinsmt":
                
                    import sys
                    sys.path.append(self.configuration_manager.module_path)  # Replace '/path/to/TransUNet' with the actual path to the TransUNet module

                    from SwinSMT.models.swin_smt import SwinSMT
                    ...
                    if self.configuration_manager.pretrained_weights is not None:
                        ...
                        dst = "SwinSMT/pretrained/ssl_pretrained_weights.pth"
                        ...
                        model_prior_dict = net.state_dict()
                        ...
                        for key, value in ssl_weights.items():
                            if key[:8] == "encoder.":
                                ...
                                # added by eli: dont load linear layer that are now experts
                                if new_key[27:37] == "mlp.linear":
                        ...
                        net.load_state_dict(model_update_dict, strict=True)
                        model_final_loaded_dict = net.state_dict()
                    ...
                    self.network = net.to(self.device)
                elif self.configuration_manager.network == "uxnet":
                    import sys
                    sys.path.append(self.configuration_manager.module_path)

                    from uxNet.networks.UXNet_3D.network_backbone import UXNET
                    ...
                    if self.configuration_manager.pretrained_weights is not None:
                        ...
                        model.load_state_dict(torch.load(self.configuration_manager.pretrained_weights))
                        model.out = UnetOutBlock(spatial_dims=3, in_channels=48, out_channels=self.label_manager.num_segmentation_heads)
                        print(f"Using pretrained UNETR backbone weights! {self.configuration_manager.pretrained_weights}")

                    else:
                        model = UXNET(
                            in_chans=self.num_input_channels,
                            out_chans=self.label_manager.num_segmentation_heads,
                            depths=[2, 2, 2, 2],
                            feat_size=[48, 96, 192, 384],
                            drop_path_rate=0,
                            layer_scale_init_value=1e-6,
                ...
                elif self.configuration_manager.network == "nnformer":
                    import sys
                    sys.path.append(self.configuration_manager.module_path)

                    from uxNet.networks.nnFormer.nnFormer_seg import nnFormer

                    net = nnFormer(input_channels=self.num_input_channels, num_classes=self.label_manager.num_segmentation_heads)
                    self.network = net.to(self.device)
    ...
    def _set_batch_size_and_oversample(self):
        if not self.is_ddp:
            ...
            print(f"finally oversampling with {oversample_percent}")
            self.print_to_log_file("oversample", oversample_percent)
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        if self.configuration_manager.network in ["TransUNet", "swinunetr", "swinunetrv2", "unetr", "swinsmt", "uxnet", "nnformer"]:
            print(f"Network is {self.configuration_manager.network}. Ignoring deep supervision.")
            return
    ...
    #@track_emissions(offline=True, country_iso_code="AUT", output_dir="emissions", output_file="10_epochs.csv", gpu_ids=[3], measure_power_secs=30)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        print("Warning currently we are only calculating the global Dice for the classes 1 and 2 for the global val metric. ")
        print(f"which is {global_dc_per_class[0:2]}")
        mean_fg_dice = np.nanmean(global_dc_per_class[0:2])
